/*
This file is part of TheKBase Desktop
A Multi-Hierarchical  Information Manager
Copyright (C) 2004-2006 Daniel Rosenstark

TheKBase Desktop is free software; you can redistribute it and/or
modify it under the terms of the GNU General Public License
("GPL") version 2 as published by the Free Software Foundation.
See the file LICENSE.TXT for the full text of the GNU GPL, or see
http://www.gnu.org/licenses/gpl.txt

For using TheKBase Desktop with software that can not be combined with 
the GNU GPL or any other queries, please contact Daniel Rosenstark 
(license@thekbase.com).
*/
using System;
using System.Collections.Generic;
using System.Text;

namespace Kbase.LibraryWrap
{

    /// <summary>
    /// This is a dummy class because we don't have the external lib for the encryption
    /// </summary>
    class XmlEncryption
    {

        static XmlEncryption() {
            ENCRYPTED_NODE_NAME = "WedNeverKnowIt";
        }

        internal static void EncryptElement(System.Xml.XmlNode xmlNode, string password, bool content)
        {
            throw new Exception("This feature is not implmemented");
        }

        internal static string ENCRYPTED_NODE_NAME = null;

        internal static void DecryptElement(System.Xml.XmlNode encryptedNode, string password)
        {
            // do nothing
        }
    }
}
