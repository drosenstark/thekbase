...
namespace Kbase.MainFrm
{
    ...
    public class MainForm : Form
    {
        ...
        private void Init2()
        {
            ...
            // propertiesPane
            Universe.Instance.propertiesPane = new Kbase.Properties.PropertiesPane();
            Universe.Instance.propertiesPane.TabIndex = 2;
            ...
            Universe.Instance.propertiesPane.Dock = DockStyle.Top;
            panel.Controls.Add(Universe.Instance.detailPane);
            panel.Controls.Add(splitter2);
            panel.Controls.Add(Universe.Instance.propertiesPane);
        ...
        void LoadMenus()
        {
            ...
            // setup FILE menu, THIS IS A MEMBER VARIABLE!
        ...
        public static bool PromptUserToRetryInServerOperation(string operation)
        {
            bool retry = false;
            DialogResult result = DialogResult.Cancel;
            DialogResult result2 = DialogResult.No;

            result = MessageBox.Show(operation + " failed, retry?", DialogCaption, System.Windows.Forms.MessageBoxButtons.RetryCancel);
            retry = (result == DialogResult.Retry);
            if (!retry)
            {
                result2 = MessageBox.Show("This will cause you to exit from TheKBase. Are you sure?", DialogCaption, MessageBoxButtons.YesNo);
                retry = (result2 == DialogResult.No);
            }
            return retry;
        }
