...
namespace Kbase.SnippetTreeView
{
    ...
    public class SnippetPane : ScrollingTreeView
    {
        ...
        protected override void SetupRightClickMenu()
        {
            ...
            menuItem.Click += new EventHandler(ClickSort);
            RightClickMenu.MenuItems.Add(menuItem);
        ...
        public void KeyPressMoveUp()
        {
            if (SelectedNodes.Count > 1)
            {
                MainForm.ShowMessage("Sorry, you cannot move multiple nodes.");
                return;
            }
            foreach (SnippetTNode node in SelectedNodes)
            {
        ...
        public void ClickSort(object sender, System.EventArgs e)
        {
            try
            {
                if (SelectedNodes.Count == 0)
                    Universe.Instance.ModelGateway.TopLevelSnippet.SortChildren();
                else
                {
                    foreach (SnippetTNode node in SelectedNodes)
                    {
                        node.Snippet.SortChildren();
                    }
                }
            }
            catch (Exception e2)
